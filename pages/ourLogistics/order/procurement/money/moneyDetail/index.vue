		...
		methods: {
			...
			handleClick(o) {
				...
				switch (o) {
					case 0:
						uni.showModal({
							...
							success: function(sm) {
								if (sm.confirm) {
									uni.request({
										url: _self.$webUrl + "/presale/order/cancelOrder",
										data: {
											id: _self.order.id
										},
										header: {
					...
					case 1:
						uni.showModal({
							...
							success: function(sm) {
								if (sm.confirm) {
									uni.request({
										url: _self.$webUrl + "/presale/order/confirmOrder",
										data: {
											id: _self.order.id
										},
										header: {
					...
					case 3:
						uni.navigateTo({
							url: '../moneyContract/index?id=' + _self.order.id,
						});
						break;
					case 4:
						this.isEnd = false;
					...
					case 5:
						uni.request({
							url: _self.$webUrl + "/presale/order/refuse",
							data: {
								id: _self.order.id,
