	...
	<view class="grace-body">
		...
		<swiper class="swiper" :current="current"  @change="swiperChange" :style="{height:tabHeight+'px'}" >
			...
			<swiper-item>
				<scroll-view scroll-y="true" @scrolltolower="scrollend" :style="{height:tabHeight+'px'}">
				<view class="swiper-item">
					<view class="ot-Dtailes-host" v-for="(item,index) in log" :key="index" >
						<view class="ot-home-qx mn">
							<text></text>
							<text>单号{{index+1}}</text>
						</view>
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			uni.request({
				url: _self.$webUrl + "/logistics/moneyLogistics",
				data: {
					id: id,
					t: t
				},
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				if (!_self.submit) {
					...
					uni.request({
						url: _self.$webUrl + "/logistics/updateRealtedPreOrder",
						data: formData,
						header: {
							'Access-Token': uni.getStorageSync("token")
						},
