...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uni.request({
				url: this.$webUrl + "/logistics/preSaleOrders",
				header: {
			...
			uni.request({
				url: this.$webUrl + "/logistics/preSaleProductList",
				header: {
					'Access-Token': uni.getStorageSync("token")
				},
				success(res) {
		...
		methods: {
			...
			handleClick(e) {
				uni.navigateTo({
					url: '../moneyDetail/index?id=' + e + "&t=1",
				});
			},
			...
			scrollend(e) {
				...
				var salesman = _self.cus[_self.cusIndex].name
				if(_self.genderIndex==0){
					product=""
				}else if(_self.cusIndex==0){
					salesman=""
				}
				uni.request({
					data: {
					...
					url: _self.$webUrl + "/logistics/preSaleOrders",
					header: {
						'Access-Token': uni.getStorageSync("token")
					},
					success(res) {
